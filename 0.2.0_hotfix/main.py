...
def sort_backtest_universe(basket):
    con = sqlite3.connect('stock_data_%s.db' %config.backtesting_time_peroid)
...
if __name__ == '__main__':
    ...
    ## 백테스팅 실시
    ## config의 trade_mode의 값에 따라서 백테스팅 모드가 달라짐
    if config.trade_mode == "overnight":
        print("[OVERNIGHT BACKTESTING]")
        backtesting = overnight_basket.Core(etf_universe, config.optmizize_1)

    elif config.trade_mode == "swing":
        print("[SWING BACKTESTING]")
