...
#Bollinger Bands  
def BBANDS(df, n):  
    MA = pd.Series(pd.rolling(df['Close'], n).mean())  
    MSD = pd.Series(pd.rolling(df['Close'], n).std())  
    b1 = 4 * MSD / MA  
...
#Keltner Channel  
def KELCH(df, n):  
    KelChM = pd.Series(pd.rolling((df['High'] + df['Low'] + df['Close']) / 3, n).mean(), name = 'KelChM_' + str(n))  
    KelChU = pd.Series(pd.rolling((4 * df['High'] - 2 * df['Low'] + df['Close']) / 3, n).mean(), name = 'KelChU_' + str(n))  
    KelChD = pd.Series(pd.rolling((-2 * df['High'] + 4 * df['Low'] + df['Close']) / 3, n).mean(), name = 'KelChD_' + str(n))  
